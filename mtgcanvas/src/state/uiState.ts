...
interface UIState { mode: UIMode; }

class UIStateStore {
  state: UIState = { mode: 'canvas' };
  listeners = new Set<()=>void>();
  setMode(mode: UIMode) { if (this.state.mode!==mode) { this.state.mode = mode; this.emit(); } }
  toggleMode() { this.setMode(this.state.mode==='canvas' ? 'table':'canvas'); }
