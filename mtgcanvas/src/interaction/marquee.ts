...
export class MarqueeSystem {
  ...
  start(globalPoint: PIXI.PointData, additive: boolean) {
    ...
    rect.zIndex = 999999;
    (rect as any).eventMode = "none";
    this.world.addChild(rect);
    (this.world as any).sortDirty = true;
    (rect as any).__lastRect = { x: ws.x, y: ws.y, w: 0, h: 0 };
    this.state = {
      start: new PIXI.Point(ws.x, ws.y),
      rect,
      additive,
      active: false,
    };
  ...
  update(globalPoint: PIXI.PointData) {
    ...
    const cur = this.world.toLocal(globalPoint);
    const x1 = this.state.start.x,
  ...
  finish() {
    ...
    const data = (g as any).__lastRect;
    const activated = s.active;
    g.destroy();
    this.state = null;
