...
function parseArgs(): Args {
  ...
  if (inputIndex === -1 || !process.argv[inputIndex + 1]) {
    console.error(
      "Usage: tsx importer.ts --input path/to/all.json [--limit N] [--format ndjson|array]",
    );
    process.exit(1);
  }
  ...
  const format =
    formatIndex !== -1
      ? (process.argv[formatIndex + 1] as "ndjson" | "array")
      : "ndjson";
...
function importArray(path: string, limit?: number) {
  ...
  try {
    ...
    console.error("Failed to parse array JSON file.", e);
    process.exit(1);
  }
...
async function main() {
  ...
  getDb();
  if (format === "array") importArray(input, limit);
  else await importNdjson(input, limit);
}
