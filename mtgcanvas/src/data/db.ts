...
export function insertCard(card: any) {
  const db = getDb();
  const stmt =
    db.prepare(`INSERT OR IGNORE INTO cards (scryfall_id, name, type_line, mana_cost, cmc, color_identity, oracle_text, layout, json_raw)
  ...
  const idRow = db
    .prepare("SELECT id FROM cards WHERE scryfall_id=?")
    .get(card.id) as { id: number };
  if (card.card_faces && Array.isArray(card.card_faces)) {
    const faceStmt = db.prepare(
      `INSERT INTO card_faces (card_id, face_index, name, mana_cost, oracle_text, image_uri) VALUES (?,?,?,?,?,?)`,
    );
    const insertMany = db.transaction((faces: any[]) => {
      faces.forEach((f: any, idx: number) =>
        faceStmt.run(
          idRow.id,
          idx,
          f.name,
          f.mana_cost || null,
          f.oracle_text || null,
          (f.image_uris && f.image_uris.normal) || null,
        ),
      );
    });
