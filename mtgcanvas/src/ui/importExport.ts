...
export interface ImportExportOptions {
  ...
  importByNames: (items: { name: string; count: number }[]) => Promise<{ imported: number; unknown: string[] }>; // performs import, returns stats
  ...
  importGroups?: (data: { groups: { name: string; cards: string[] }[]; ungrouped: string[] }) => Promise<{ imported: number; unknown: string[] }>;
  // Optional: Scryfall search integration – when provided, panel shows a Search tab
  scryfallSearchAndPlace?: (query: string, opt: { maxCards?: number; groupName?: string; onProgress?: (fetched:number, total?:number)=>void; signal?: AbortSignal }) => Promise<{ imported: number; error?: string }>;
  // Optional: Debug helper to clear persisted data (positions, groups, imported cards)
  clearPersistedData?: () => Promise<void>;
}

export interface ImportExportAPI { show(): void; hide(): void; }

function groupCounts(names: string[]): { name: string; count: number }[] {
  const map = new Map<string, number>();
  for (const n of names) {
    const key = (n || '').trim(); if (!key) continue;
...
function parseDecklist(text: string): { name: string; count: number }[] {
  ...
  for (let raw of lines) {
    ...
    const hash = line.indexOf('#'); if (hash >= 0) line = line.slice(0, hash).trim();
    if (!line) continue;
    // Accept variants: "3 Lightning Bolt", "Lightning Bolt x3", or just name
    let m = line.match(/^(\d+)\s+(.+)$/);
    if (m) { out.push({ count: Math.max(1, parseInt(m[1], 10)), name: m[2].trim() }); continue; }
    m = line.match(/^(.+?)\s*[xX]\s*(\d+)$/);
    if (m) { out.push({ name: m[1].trim(), count: Math.max(1, parseInt(m[2], 10)) }); continue; }
...
function parseGroupsText(text: string): { groups: { name: string; cards: string[] }[]; ungrouped: string[] } | null {
  const lines = text.split(/\r?\n/);
  ...
  for (const raw of lines) {
    ...
    if (line.startsWith('#')) {
      ...
      if (!name) { current = null; inUngrouped = false; continue; }
      if (/^ungrouped$/i.test(name)) { current = null; inUngrouped = true; continue; }
      inUngrouped = false;
      current = { name, cards: [] };
      groups.push(current);
      continue;
...
export function installImportExport(opts: ImportExportOptions): ImportExportAPI {
  ensureThemeStyles();
  ...
  function ensure() {
    ...
    const el = createPanel({ width: 'min(1100px, 90vw)', maxHeight: '86vh', scroll: true, pointer: true });
    el.id = 'import-export-panel';
    el.style.left = '50%'; el.style.top = '10%'; el.style.transform = 'translateX(-50%)'; el.style.zIndex = '10030';
  el.innerHTML = `
  ...
  const clearBtn = el.querySelector('#ie-clear-data') as HTMLButtonElement | null;
    ...
    scopeAllEl.onchange = () => { scopeAll = true; refreshExport(); };
    scopeSelEl.onchange = () => { scopeAll = false; refreshExport(); };
  refreshBtn.onclick = () => refreshExport();
  formatSel.onchange = () => refreshExport();
    copyBtn.onclick = async ()=> { if (!exportArea) return; try { await navigator.clipboard.writeText(exportArea.value); (copyBtn as any).textContent = 'Copied'; setTimeout(()=> copyBtn.textContent='Copy', 1200); } catch {} };
    dlBtn.onclick = ()=> { if (!exportArea) return; const blob = new Blob([exportArea.value], { type: 'text/plain;charset=utf-8' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'mtg-export.txt'; document.body.appendChild(a); a.click(); setTimeout(()=> { URL.revokeObjectURL(url); a.remove(); }, 0); };
    closeBtn.onclick = ()=> hide();
    if (clearBtn && opts.clearPersistedData) {
      clearBtn.onclick = async ()=> {
        const ok = window.confirm('Clear persisted MTGCanvas data (positions, groups, imported cards)?\nThis will reload the page.');
        if (!ok) return;
        clearBtn.disabled = true; clearBtn.textContent = 'Clearing…';
  try { await opts.clearPersistedData!(); } catch {}
    ...
    importBtn.onclick = async ()=> {
      ...
      if (asGroups && opts.importGroups) {
        ...
        if (statusEl) statusEl.textContent = `Imported ${res.imported}. ${res.unknown.length? 'Unknown: '+res.unknown.join(', '): 'All resolved.'}`;
        return;
      }
      // Fallback to decklist format
      const items = parseDecklist(raw);
      if (!items.length) { if (statusEl) statusEl.textContent = 'Nothing to import.'; return; }
    ...
    if (opts.scryfallSearchAndPlace && scryPane) {
      const runBtn = el.querySelector('#ie-scry-run') as HTMLButtonElement | null;
      const qEl = el.querySelector('#ie-scry-query') as HTMLInputElement | null;
      const scryStatus = el.querySelector('#ie-scry-status') as HTMLDivElement | null;
      let scryInFlight = false;
      let scryAbort: AbortController | null = null;
      function setScryBusy(busy: boolean) {
        scryInFlight = busy;
        if (qEl) qEl.disabled = busy;
        if (runBtn) { runBtn.disabled = false; runBtn.textContent = busy ? 'Cancel' : 'Import'; }
      }
      async function runOrCancel(){
        if (scryInFlight) {
          // Treat click as cancel
          try { scryAbort?.abort(); } catch {}
          return;
        }
        const q = (qEl?.value || '').trim(); if (!q) { if (scryStatus) scryStatus.textContent='Enter a query.'; return; }
        const groupName = q.slice(0, 64);
        if (scryStatus) scryStatus.textContent = 'Searching…';
        scryAbort = new AbortController();
        setScryBusy(true);
        try {
          const res = await opts.scryfallSearchAndPlace!(q, { groupName, signal: scryAbort.signal, onProgress: (fetched, total)=> {
            if (!scryStatus) return;
            if (typeof total === 'number' && total > 0) scryStatus.textContent = `Fetching ${fetched}/${total}…`;
            else scryStatus.textContent = `Fetching ${fetched}…`;
          }});
          scryStatus && (scryStatus.textContent = res.error ? res.error : `Imported ${res.imported} cards.`);
        } catch (e:any) {
          if (e && (e.name === 'AbortError' || /aborted/i.test(e.message||''))) {
            scryStatus && (scryStatus.textContent = 'Canceled.');
          } else {
            scryStatus && (scryStatus.textContent = 'Search failed: ' + (e?.message || String(e)));
          }
        } finally {
          setScryBusy(false);
          scryAbort = null;
      ...
      qEl?.addEventListener('keydown', (ev)=> { if (ev.key==='Enter' && !ev.shiftKey) { ev.preventDefault(); if (!scryInFlight) runOrCancel(); } });
    }
  ...
  function show(){ const el = ensure(); el.style.display='block'; // pre-populate export
  ...
  window.addEventListener('keydown', (e)=> { if (e.key==='Escape' && panel && panel.style.display!=='none') hide(); }, { capture: true });
