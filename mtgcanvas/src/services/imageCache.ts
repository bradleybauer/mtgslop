...
const BUDGET_MB = BUDGET_MB_RAW === null ? Number.POSITIVE_INFINITY : Number(BUDGET_MB_RAW);
const BYTE_BUDGET = !isFinite(BUDGET_MB) ? Number.POSITIVE_INFINITY : BUDGET_MB * 1024 * 1024;
export async function getCacheUsage(): Promise<{count:number; bytes:number; budget:number; over:boolean}> {
  try {
    ...
    await new Promise<void>((resolve, reject)=> {
      ...
      cursor.onsuccess = () => { const c = cursor.result; if (!c){ resolve(); return; } const v:any = c.value; bytes += v.size||0; count++; c.continue(); };
      cursor.onerror = () => reject(cursor.error!);
    });
    return { count, bytes, budget: BYTE_BUDGET, over: isFinite(BYTE_BUDGET) ? bytes>BYTE_BUDGET : false };
  } catch { return { count:0, bytes:0, budget: BYTE_BUDGET, over:false }; }
}

interface CacheEntryMeta { url:string; blob:Blob; ctype:string; size:number; time:number; }
...
function dbg(...args:any[]){ if (debugEnabled) console.log('[imageCache]', ...args); }
export function enableImageCacheDebug(on:boolean){ debugEnabled = on; localStorage.setItem('imgCacheDebug', on?'1':'0'); }

// Concurrency limiter for network fetches (avoid browser resource exhaustion)
const MAX_FETCH_CONCURRENCY = Number(localStorage.getItem('imgFetchConcurrency') || '32');
let activeFetches = 0; const waitQueue: (()=>void)[] = [];
function acquireFetchSlot(): Promise<void> {
  if (activeFetches < MAX_FETCH_CONCURRENCY) { activeFetches++; return Promise.resolve(); }
  return new Promise(res=> waitQueue.push(()=> { activeFetches++; res(); }));
}
function releaseFetchSlot(){ activeFetches--; if (activeFetches<0) activeFetches=0; const next = waitQueue.shift(); if (next) next(); }

export function getImageCacheStats(){
  const avg = netFetches? (netBytes/netFetches) : 0;
  return { sessionHits, idbHits, netFetches, netBytes, avgFetchKB: avg/1024, lastNetMs, canonicalHits, netErrors, resourceErrors, activeFetches, queuedFetches: waitQueue.length, duplicateCanonicals, unexpectedSmallBlob };
}
...
async function fetchAndCache(url:string): Promise<CachedImage> {
  ...
  try {
    ...
    const meta: CacheEntryMeta = { url: canonical, blob, ctype: resp.headers.get('content-type') || '', size: blob.size, time: Date.now() };
    putEntry(meta);
...
export async function clearImageCache() {
  try { const db = await openDB(); const tx = db.transaction(STORE,'readwrite'); tx.objectStore(STORE).clear(); } catch {}
  objectUrlCache.forEach(u=> URL.revokeObjectURL(u)); objectUrlCache.clear();
}
